...
export interface CreatorProfile { name: string | null; avatarUrl: string | null }
...
export function createProfileLookup(request: typeof fetch = fetch) {
  ...
  return (input: string): Promise<CreatorProfile> => {
    ...
    const entry = { expires: Date.now() + 3600000, value: Promise.resolve<CreatorProfile>({ name: null, avatarUrl: null }) };
    entry.value = (async () => {
      try {
        ...
        const data = z.object({ code: z.literal(0), data: z.object({ card: z.object({ name: z.string().max(200), face: z.string().url() }) }) }).parse(await response.json());
        ...
        return { name: data.data.card.name, avatarUrl: avatar.href };
      } catch {
