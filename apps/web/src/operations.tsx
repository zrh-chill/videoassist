...
export function CreatorsPage() {
  ...
  return <>
    ...
    {editor && <CreatorEditor key={editor === 'add' ? 'add' : editor.id} creator={editor === 'add' ? null : editor} onClose={() => setEditor(null)} onSaved={id => {
      refresh(); setEditor(null); setCollapsed(previous => ({ ...previous, [id]: false })); setNotice(editor === 'add' ? 'UP 主已添加' : '追踪设置已保存');
    }}/>}
...
function CreatorEditor({ creator, onClose, onSaved }: { creator: Creator | null; onClose: () => void; onSaved: (id: string) => void }) {
  ...
  return <dialog ref={dialog} className="creator-modal" aria-labelledby="creator-modal-title" onCancel={event => { event.preventDefault(); if (!save.isPending) onClose(); }} onClick={event => { if (event.target === event.currentTarget && !save.isPending) { const r = event.currentTarget.getBoundingClientRect(); if (event.clientX < r.left || event.clientX > r.right || event.clientY < r.top || event.clientY > r.bottom) onClose(); } }}>
    <form onSubmit={event => { event.preventDefault(); save.mutate(event.currentTarget); }}><div className="modal-head"><h2 id="creator-modal-title">{creator ? '追踪设置' : '添加 UP 主'}</h2><button type="button" className="icon-btn" aria-label="关闭弹窗" disabled={save.isPending} onClick={onClose}>×</button></div>
      ...
      {save.error && <p className="error" role="alert">{save.error.message}</p>}</div><div className="modal-foot"><button type="button" className="btn" disabled={save.isPending} onClick={onClose}>取消</button><button className="btn primary" disabled={save.isPending}>{save.isPending ? '正在保存…' : creator ? '保存设置' : '添加 UP 主'}</button></div>
    </form></dialog>;
}
...
function CreatorCard({ creator, pending, open, onToggle, onHistory, onAction, onEdit }: { creator: Creator; pending: boolean; open: boolean; onToggle: () => void; onHistory: () => void; onAction: (name: 'check' | 'edit' | 'delete', body?: unknown) => void; onEdit: () => void }) {
  const [avatarFailed, setAvatarFailed] = useState(false);
  const profile = useQuery({ queryKey: ['creator-profile', creator.id], queryFn: () => api<{ name: string | null; avatarUrl: string | null }>('/creators/' + creator.id + '/profile'), staleTime: 3600000, refetchInterval: false, retry: false });
  const displayName = profile.data?.name || creator.name;
  const videos = useQuery({ queryKey: ['videos', 'creator-preview', creator.id], queryFn: () => api<VideoPage>('/videos?creatorId=' + creator.id + '&limit=5'), refetchInterval: 5000 });
  const checks = useQuery({ queryKey: ['creator-checks', creator.id], queryFn: () => api<Operation[]>('/creators/' + creator.id + '/checks'), refetchInterval: 3000 });
  ...
  return <section className={'creator-group' + (open ? ' open' : '')}>
    <div className="creator-card"><a className="avatar" href={creator.url} target="_blank" rel="noreferrer" aria-label={'打开 ' + displayName + ' 主页'}>{profile.data?.avatarUrl && !avatarFailed ? <img src={profile.data.avatarUrl} alt={displayName + '的头像'} referrerPolicy="no-referrer" onError={() => setAvatarFailed(true)}/> : <span title="头像暂不可用">{displayName.slice(0, 1)}</span>}</a>
      <div className="creator-identity"><h3><a href={creator.url} target="_blank" rel="noreferrer">{displayName}</a></h3><p>UID {creator.uid} · {creator.lastCheckedAt ? '上次检查 ' + new Date(creator.lastCheckedAt).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '尚未检查'} · {creator.enabled ? (creator.autoProcess ? '自动处理已开启' : '手动处理') : '已暂停'}</p></div>
      ...
      <button className="icon-btn" aria-label={'设置 ' + displayName} title="追踪设置" disabled={pending} onClick={onEdit}>⚙</button>
    </div>
    <div className="creator-management"><button className="btn small" onClick={onHistory}>检查记录</button><button className="btn small" disabled={pending} onClick={() => onAction('edit', { enabled: !creator.enabled })}>{creator.enabled ? '暂停追踪' : '启用追踪'}</button><button className="btn small danger" disabled={pending} onClick={() => { if (window.confirm('停止追踪该 UP 主？已导入视频和检查记录会保留。')) onAction('delete'); }}>停止追踪</button></div>
