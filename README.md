...
已完成技术方案的**阶段 2：本地视频闭环**，并根据测试链接提前接入阶段 3 的 B 站单视频解析、下载和 BVID 去重。
...
- B 站标准单视频 URL 导入、元数据与下载、Cookie 文件引用、BVID 去重。
...
测试使用临时数据库、FFmpeg 合成视频和本机 HTTP 模型桩，不调用真实模型，不读取实际业务数据。测试环境也需要 FFmpeg/FFprobe。测试覆盖上传限制、BVID 并发去重、分片续传、时间偏移、版本保留、长文稿、模型错误、持久任务和 SSE。
...
转写分片和 Map 结果按音频哈希、模型、输入及提示词等生成检查点键；读取媒体时校验文件存在及哈希。缺失或被替换的文件会明确失败，不继续调用模型。远端调用已完成但本地检查点尚未提交时崩溃，仍可能再次计费。
...
- B 站仅支持 https 标准 BV 单视频链接及第一分 P；短链接、多分 P、重新下载及完整下游失效策略仍待阶段 3 补齐。
- 模型参数、工具路径和提示词通过服务端 .env/文件配置；设置编辑、连接测试和提示词管理界面留在阶段 4。
- 长文稿使用 cl100k_base 估算令牌并额外保留 20% 余量；不同模型分词不同，可调节预算。
- Excel 导出、UP 主追踪、自动清理和备份管理未实现，不将当前状态视为完整 V1。
